...
import FirestoreDbService, { IFirestoreDbService } from "../src";
...
describe(`Feathers common tests, ${serviceName} service with \\${idProp}\\ id property `, () => {
  ...
  const testPass = 'root';
  let app: Application;
  let service: IFirestoreDbService<any>;
  let _ids: any = {};
  ...
  beforeAll(async () => {
    ...
    service = <IFirestoreDbService<any>>app.service(serviceName);
  });
  ...
  it('Accepts string as database & collection', async () => {
    const dbService = FirestoreDbService({
      id: idProp,
    });
